...
- **한상원**
> - 막연히 PhotoSwuipe와 같은 프로그램을 만들려고 했으면 불가능 했을 것입니다. 하지만, 이번 오픈소스 과목을 통해 Github 사이트와 오픈소스 활용법 빌드 해보는 법, 그리고 프로젝트에 있어서 가장 중요한 협업을 가능하게 해줘 프로그래밍 개발이 좀 더 향상 되었던 것 같습니다. PhotoSwipe 오픈소스를 선택하여 이 프로그램의 모션을 통한 사진이동이 가장 기억에 남는 효율적인 기능이라고 생각합니다. PhotoSwiper기능향상에 사진에 그림이나 글자를 적을
수 있는 기능과 제한된 앨범 크기 안에 여러장의 사진을 넣을 수 있는 기능을 추가하게된다면 좀 더 PhotoSwipe프로그램의 기능이 향상 될 것 같다고 생각합니다. 무엇보다 Github에서 조원들이 무엇을 했는지 어떻게 했는지를 보면서 참고 할 수 있다는 점이 프로젝트에 있어서 가장 효율적이였다고 생각합니다. 프로젝트에 참여하고있다는 소속감도 들어서 보람찼습니다.
